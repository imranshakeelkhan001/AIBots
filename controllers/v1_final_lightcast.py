...
class CalendarEvent(BaseModel):
    justification_of_job_description: Optional[str] = None
    canvas_window: bool
    # post_job: bool
    linkedin_job: bool
    indeed_job: bool
    career_page_post: bool
...
def canvas_flag(history):
    history[0] = {
        "role": "system",
        "content": (
            "canvas_window: Turn this flag to True only when complete job description is generated by assistant.and 'False' otherwise."
            "once complete job description is generated. than generate reasoning in 'justification_of_job_description' and justify generation of each section of the job description, that How you reached out to generate it."
            "If 'canvas_window' is 'False', set 'justification_of_job_description' to 'None'."
            # "extract the second boolean flag 'post_job' from the give chat if the user requested to post the job description and the assistant responded 'where you want to post the job description."
            "career_page_post: Turn this flag True if user says post the job description on our career page"
            "linkedin_job: Turn this flag True if user says post the job description to on linkedin"
            "indeeed_job: Turn this flag True if user says post the job description to on indeed"
        )
    }
    ...
    # **Ensure justification_of_job_description is None when canvas_window is False**
    if not event.canvas_window:
        event.justification_of_job_description = None
...
def list_of_job_requisitions():
    url = "http://af8f9979b16504831a400599cb562ea7-2055552796.eu-north-1.elb.amazonaws.com:8080/JobRequisition/approvedRequisitions"
    ...
    # Remove the 'summary' key from each item in the list
    for item in data:
        item.pop("summary", None)  # Remove 'summary' if it exists

    # Print the filtered response
    print("here is list of job requisitions", data)
    return data
...
system_message = {
        ...
        "content": (
	        ...
	        f"you are provided with list of job requisitions, when you receive title from user for creating job description. You will use that title and confirm the details from user for the data that you have in list of job requisition for that specific job title."
            f"You have In list of job requisition you have data of ‘department, qualification, job timing, and experience.’ "
            f"for example if user says create a job description for ai engineer, and you have data in list of job requisition for this job title."
            f"you must confirm the details from the user e.g ‘the department for the ai engineer is engineering department right?"
            "and than you confirm ‘the qualification for this is MS Computer science, right?’"
            "and than you confirm ‘the job timing for this job is 9 to 5’ is it right?"
            "and than you confirm ‘ the experience for this job is 5 years right?’"
            f"Please Note: you must use these details and you will confirm it by asking from user one by one in a conversational manner."
             "IMPORTANT!! In any case, must ask him specific questions for the job role following are the example questions for an imaginary job role i.e. chemical engineer, but do not stick to these questions only"
            f"for rest of the questions you will ask as it is."
            ...
            " and your response should be conversational throughout  "
            f"Please Note that, Do not ask Multiple questions in 1 response."
            ...
            "and user can even edit your provided job description. after finalizing the job description you should ask your  Shall I proceed to distribute the job posting? if user says yes than respond to user that 'where you want to post the job description.e.g Linkedin,indeed, on our career page?'"
            "if user says on our career page, you will respond that 'it will be posted in a while.'"
            "if user says on Linkedin you will respond that it will be posted on linkedin in a while, thank you."
            "if user says on indeed  you will respond that it will be posted on indeed in a while,thank you."
...
async def jd_generator(query, id):
    message = []
    list_job_requisitions = {"role": "assistant", "content": f"here is list of job requisitions {str(list_of_job_requisitions())}"}
    message.append(list_job_requisitions)
