...
async def extract_data_from_cv(jd_id,skill_factor, cv_file: UploadFile):
    ...
    try:
        ...
        jd = get_job_by_id(jd_id)
        generated_skills = skill_generator(get_job_by_id(jd_id))
        # skill_score, matched_skills = jaccard_similarity_skills(generated_skills,skill_cv)
        # print("skill score",skill_score)
        print("generated skil", generated_skills)
        print("resume skills ", skill_cv)

        # euc_dist = euclidean_distance_bert(str(generated_skills), str(skill_cv))
        matched_skills, number_of_skills = getmatch(generated_skills, skill_cv)
        response['skil_score'] = number_of_skills/len(generated_skills)
        rank = cosine_similarity_sklearn(str(cv_text),str(jd))

        response['rank'] = (float(rank)*100)*(response['skil_score']*skill_factor)
        response['matched_skills'] = matched_skills
        response['resume_json'] = cv_text
        chat.append({"role":"user", "content":f"the following is the job details {jd}"})
        ...
        print(response['skil_score']/skill_factor)
