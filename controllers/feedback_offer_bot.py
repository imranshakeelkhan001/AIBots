...
def canvas_flag(history):
    ...
    # Step 2: If not found, use OpenAI API with refined prompt
    history.insert(0, {"role": "system", "content": 
        "Extract boolean flags from the given conversation: "
        "- 'feed_back': True if the assistant asked 'how was the feedback for the candidate?'. "
        "- 'offer_letter': True if the assistant generated a complete offer letter. "
        "- 'offer_send': True if the user approved and confirmed sending the offer letter."
    })
    ...
    try:
        ...
        return {"feed_back": False, "offer_letter": False, "offer_send": False}  # Default safe response
